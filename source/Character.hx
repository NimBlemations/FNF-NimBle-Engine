...
class Character extends FlxSprite
{
	...
	// Just because
	public var cameraOffsets:Map<String, Array<Int>>;
	public var cameraOffset:FlxPoint;
	public var debugMode:Bool = false;

	public var isPlayer:Bool = false;
	public var curCharacter:String = 'bf';
	public var barColor:FlxColor = 0xFFaaaaaa;
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		cameraOffsets = new Map<String, Array<Int>>();
		
		cameraOffset = new FlxPoint();
		
		// Fuck.
		cameraOffsets.set('singLEFT', [-20, 0]);
		cameraOffsets.set('singDOWN', [0, 20]);
		cameraOffsets.set('singUP', [0, -20]);
		cameraOffsets.set('singRIGHT', [20, 0]);
		cameraOffsets.set('idle', [0, 0]);
		cameraOffsets.set('singLEFTmiss', [-20, 0]);
		cameraOffsets.set('singDOWNmiss', [0, 20]);
		cameraOffsets.set('singUPmiss', [0, -20]);
		cameraOffsets.set('singRIGHTmiss', [20, 0]);
		cameraOffsets.set('singLEFTalt', [-20, 0]);
		cameraOffsets.set('singDOWNalt', [0, 20]);
		cameraOffsets.set('singUPalt', [0, -20]);
		cameraOffsets.set('singRIGHTalt', [20, 0]);
		...
		switch (curCharacter)
		{
			...
			case 'monster':
				...
				loadOffsetFile(curCharacter);
				loadCamOffsetFile(curCharacter);
				barColor = 0xFFf3ff6e;
			...
			case 'bf':
				var tex = Paths.getSparrowAtlas('characters/BOYFRIEND_CS6'); // CS6 and Adobe Animate spritesheets don't line up apparently.
				frames = tex;
				quickAnimAdd('idle', 'BF idle dance');
				quickAnimAdd('singUP', 'BF NOTE UP0');
				quickAnimAdd('singLEFT', 'BF NOTE LEFT0');
				quickAnimAdd('singRIGHT', 'BF NOTE RIGHT0');
				quickAnimAdd('singDOWN', 'BF NOTE DOWN0');
				quickAnimAdd('singDOWN-alt', 'BF NOTE DOWN ALT0');
				quickAnimAdd('singUPmiss', 'BF NOTE UP MISS');
				quickAnimAdd('singLEFTmiss', 'BF NOTE LEFT MISS');
				quickAnimAdd('singRIGHTmiss', 'BF NOTE RIGHT MISS');
				quickAnimAdd('singDOWNmiss', 'BF NOTE DOWN MISS');
	...
	private function loadCamOffsetFile(offsetCharacter:String)
	{
		var daFile:Array<String> = CoolUtil.coolTextFile(Paths.file("images/characters/" + offsetCharacter + "CamOffsets.txt"));
		
		for (i in daFile)
		{
			var splitWords:Array<String> = i.split(" ");
			cameraOffsets[splitWords[0]] = [Std.parseInt(splitWords[1]), Std.parseInt(splitWords[2])];
		}
	}
	...
	public function playAnim(AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0):Void
	{
		...
		var daCamOffset = cameraOffsets.get(AnimName);
		if (cameraOffsets.exists(AnimName))
		{
			if (cameraOffset.x != daCamOffset[0] || cameraOffset.y != daCamOffset[1])
			{
				cameraOffset.set(daCamOffset[0], daCamOffset[1]);
			}
	...
	{
		animOffsets.set(name, [Std.int(x), Std.int(y)]);
	}
}

...
import shaderslmfao.ImageDuo;
import throwables.ComboSpr;
...
class PlayState extends MusicBeatState
{
	...
	private var endingSong:Bool = false; // No re-looping!
	...
	private var totalNotesPassed:UInt = 0;
	private var noteAccPercent:Float = 0;
	
	var playerBotplay:Bool = PreferencesMenu.getPref('player-botplay');

	var grpNoteSplashes:FlxTypedGroup<NoteSplash>;
	var grpCombo:FlxTypedGroup<ComboSpr>;
	...
	var songLength:Float = 0; // Need for my copy+paste resync
	...
	var discordUpdate:Float = 1 / 3; // 1 divided by Frames Per Second
	var discordClicker:Float = 0;
	#end
	
	private var prefHaltLag:Bool = PreferencesMenu.getPref('lag-halt');
	private var prefRecycleSprites:Bool = PreferencesMenu.getPref('recycle-sprites');
	private var prefNoteSplash:Bool = PreferencesMenu.getPref('strum-splash');
	private var prefCamNoteMove:Bool = PreferencesMenu.getPref('camera-move-note');
	private var updateAccThing:Bool = PreferencesMenu.getPref('ui-accuracy');
	...
	override public function create()
	{
		...
		FlxG.save.bind('funkin', 'nimblemations');
		...
		// v New preload v
		Paths.preloadSound('songs', '${SONG.song.toLowerCase()}/Inst');
		if (SONG.needsVoices)
			Paths.preloadSound('songs', '${SONG.song.toLowerCase()}/Voices');
		...
		if (curStage == 'spooky')
		{
			var buttmap = Paths.bitmap('characters/BoyFriend_Assets_Thudner');
			imageDuoShady = new ImageDuo(buttmap);
			boyfriend.shader = imageDuoShady.shader;
		}
		...
		grpCombo = new FlxTypedGroup<ComboSpr>();
		...
		scoreTxt = new FlxText((updateAccThing ? healthBarBG.x + (healthBarBG.width / 6) : healthBarBG.x + healthBarBG.width - 190), healthBarBG.y + 30, 0, "", 20);
		if (updateAccThing)
			scoreTxt.screenCenter(X);
		scoreTxt.scrollFactor.set();
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, (updateAccThing ? CENTER : RIGHT), FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(scoreTxt);
	...
	function initDiscord():Void
	{
		...
		detailsText = isStoryMode ? "Story Mode: Week " + storyWeek : "Freeplay" + '; ${SONG.song} ($storyDifficultyText) ' + (playerBotplay ? "\n(BOTPLAY)" : "");
		detailsPausedText = "Paused - " + detailsText;

		// Updating Discord Rich Presence.
		DiscordClient.changePresence(detailsText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC);
		#end
	}
	...
	function startCountdown():Void
	{
		for (i in 0...10)
		{
			if (curStage.startsWith("school"))
				Paths.preloadGraphic("weeb/pixelUI/" + 'num' + i + "-pixel");
			else
		...
		{
			var comboStuff:Array<String> = ['sick', 'good', 'bad', 'shit', 'combo'];
			
			if (curStage.startsWith("school"))
			{
				for (i in 0...comboStuff.length)
				{
					var thingString = comboStuff[i];
					comboStuff[i] = "weeb/pixelUI/" + thingString + '-pixel';
				}
			}
		...
		startTimer.start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			for (i in introSprPaths)
			{
				Paths.preloadGraphic(i);
			}
			for (i in introSndPaths)
			{
				Paths.preloadSound('sounds', i);
			}
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC, true, songLength);
		#end
	}
	...
	{	
		...
		trace('Time for sorting things out...');
		
		var previousNote:Note = null;
		for (swaggimus in unspawnNotes) // All I want in this loop is detailing lmao
		{
			var dist:Float = 1;
			for (swaggithus in unspawnNotes)
			{
				if (swaggimus != swaggithus && swaggithus.strumTime != swaggimus.strumTime && swaggimus.strumTime > swaggithus.strumTime)
				{
					var relativeDist:Float = Math.abs((swaggithus.strumTime / Conductor.bpm / 16) - (swaggimus.strumTime / Conductor.bpm / 16));
					
					relativeDist *= 2; // Pointless, I know, but I kinda want to keep the 16's
					// Without this line above, the inputs seem more realistic to an expert player just straight tapping the keys fast, but I don't know if I want to
					
					if (swaggimus.isSustainNote || swaggithus.isSustainNote)
						relativeDist *= 2;
					
					if (relativeDist < dist)
						dist = relativeDist;
				}
				else if (swaggimus.noteData < swaggithus.noteData && swaggithus.strumTime == swaggimus.strumTime && swaggimus.mustPress == swaggithus.mustPress)
				{
					swaggithus.animate = false;
					#if debug
					trace('gotcha');
					#end
				}
			}
			swaggimus.timePressing = Math.min(32 / Conductor.bpm, dist);
		}
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			#if discord_rpc
			if (startTimer.finished)
				DiscordClient.changePresence(detailsText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC, true, songLength - Conductor.songPosition);
			else
				DiscordClient.changePresence(detailsText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC);
			#end
		}
	...
	{
		if (health > 0 && !paused && FlxG.autoPause)
			DiscordClient.changePresence(detailsPausedText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC);
	...
	var opponentTimerArray:Array<Float> = [0, 0, 0, 0]; // Making the opponent's arrow unglow quicker.
	...
	override public function update(elapsed:Float)
	{
		...
		var flxElapsed:Float = FlxG.elapsed * 1000;
		if (FlxG.elapsed >= 1 / 10 && prefHaltLag) // Lag compensation, so it doesn't go NEEEEOOOOOOOOOOM!!!!! And also, I'm really annoyed by lag that affects the time elapsed.
		#if debug
		{
			trace('LAGGY!!!!');
			#end
			flxElapsed *= FlxG.elapsed / 2;
			#if debug
		}
		#end
		...
		if (startingSong)
		{
			if (startedCountdown)
			{
				Conductor.songPosition += flxElapsed;
		...
		scoreTxt.text = 'Score:$songScore' + (updateAccThing ? ' | Misses:$misses | Accuracy:${accuracy}%' : "");
		
		#if discord_rpc
		if (discordClicker >= discordUpdate)
		{
			DiscordClient.changePresence(detailsText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC, true, songLength - Conductor.songPosition);
			discordClicker = 0;
		}
		else
			discordClicker += elapsed;
		#end
		...
		if (!inCutscene && !_exiting)
		{
			...
			if (health <= 0 && !practiceMode)
			{
				...
				DiscordClient.changePresence("Game Over - " + detailsText, 'Score:$songScore' + (updateAccThing ? ' - Miss:$misses - Acc:${accuracy}%' : ""), iconRPC);
				#end
			}
		}
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && !daNote.wasGoodHit && daNote.strumTime <= Conductor.songPosition)
				{
					...
					opponentStrums.forEachAlive(function(staticNote:FlxSprite) // doin cool shit (vs. joe)
					{
						if (staticNote.ID == daNote.noteData)
						{
							opponentTimerArray[daNote.noteData] = daNote.timePressing;
							staticNote.animation.play('confirm', true);
							if (daNote.noteStatus > 1)
							{
								#if debug
								if (!opponentSusArray[daNote.noteData])
				...
				if (daNote.mustPress && daNote.strumTime <= Conductor.songPosition && !daNote.isSustainNote && !daNote.wasGoodHit && playerBotplay)
				{
					goodNoteHit(daNote);
				}
			...
			opponentStrums.forEachAlive(function(staticNote:FlxSprite) // doin cool shit (vs. joe)
			{
				if ((staticNote.animation.curAnim.finished || opponentTimerArray[staticNote.ID] <= 0) && staticNote.animation.curAnim.name == 'confirm' && !opponentSusArray[staticNote.ID])
				{
		...
		for (time in 0...opponentTimerArray.length)
		{
			if (opponentTimerArray[time] > 0)
				opponentTimerArray[time] -= elapsed;
		}
		
		if (playerBotplay)
		{
			for (time in 0...playerTimerArray.length)
			{
				if (playerTimerArray[time] > 0)
					playerTimerArray[time] -= elapsed;
			}
		}
	...
	{
		#if debug
		var acc:Float = noteAccPercent / totalNotesPassed * 100;
		FlxG.watch.addQuick("acc", acc);
		accuracy = FlxMath.roundDecimal(acc, 2);
		#else
		accuracy = FlxMath.roundDecimal(noteAccPercent / totalNotesPassed * 100, 2);
		#end
	}
	...
	{
		var noteActualDiff:Float = strumtime - Conductor.songPosition;
		var noteDiff:Float = Math.abs(noteActualDiff);
		// boyfriend.playAnim('hey');
		vocals.volume = 1;

		var rating:ComboSpr;
		if (prefRecycleSprites)
		{
			rating = grpCombo.recycle(ComboSpr);
			rating.setup(0, 0, Conductor.crochet * 0.001);
		}
		else
		{
			rating = new ComboSpr(0, 0, Conductor.crochet * 0.001);
			rating.memory = false;
		}
		var score:Int = 350;
		...
		if (updateAccThing)
		{
			var wife:Float = EtternaFunctions.wife3(noteActualDiff);
			noteAccPercent += wife;
		}

		var isSick:Bool = (noteDiff < Conductor.safeZoneOffset * 0.2); // Heh, just cleaned up the code.
	...
	function displayCombo():Void
	{
		...
		var comboSpr:ComboSpr;
		if (prefRecycleSprites)
		{
			comboSpr = grpCombo.recycle(ComboSpr);
			comboSpr.setup(FlxG.width * 0.55, FlxG.camera.scroll.y + FlxG.camera.height * 0.4 + 80, Conductor.crochet * 0.001);
		}
		else
		{
			comboSpr = new ComboSpr(FlxG.width * 0.55, FlxG.camera.scroll.y + FlxG.camera.height * 0.4 + 80, Conductor.crochet * 0.001);
			comboSpr.memory = false;
		}
		...
		*/
		...
		{
			var numScore:ComboSpr;
			if (prefRecycleSprites)
			{
				numScore = grpCombo.recycle(ComboSpr);
				numScore.setup(0, 0, Conductor.crochet * 0.002);
			}
			else
			{
				numScore = new ComboSpr(0, 0, Conductor.crochet * 0.002);
				numScore.memory = false;
			}
	...
	{
		var dadCamOffset:FlxPoint = (prefCamNoteMove ? dad.cameraOffset : Const.FlxPZero);
		var bfCamOffset:FlxPoint = (prefCamNoteMove ? boyfriend.cameraOffset : Const.FlxPZero);
		if (/* camFollow.x != (dad.getMidpoint().x + 150 + dad.cameraOffset.x) && */ !cameraRightSide /* && camFollow.y != (dad.getMidpoint().y - 100 + dad.cameraOffset.y) */)
		{
			camFollow.setPosition(dad.getMidpoint().x + 150 + dadCamOffset.x, dad.getMidpoint().y - 100 + dadCamOffset.y);
			// camFollow.setPosition(lucky.getMidpoint().x - 120, lucky.getMidpoint().y + 210);

			switch (dad.curCharacter)
			{
				case 'mom':
					camFollow.y = dad.getMidpoint().y + dadCamOffset.y;
				case 'senpai' | 'senpai-angry':
					camFollow.y = dad.getMidpoint().y - 430 + dadCamOffset.y;
					camFollow.x = dad.getMidpoint().x - 100 + dadCamOffset.x;
			}
		...
		if (cameraRightSide /* && camFollow.x != (boyfriend.getMidpoint().x - 100 + boyfriend.cameraOffset.x) && camFollow.y != (boyfriend.getMidpoint().y - 100 + boyfriend.cameraOffset.y) */)
		{
			camFollow.setPosition(boyfriend.getMidpoint().x - 100 + bfCamOffset.x, boyfriend.getMidpoint().y - 100 + bfCamOffset.y);

			switch (curStage)
			{
				case 'limo':
					camFollow.x = boyfriend.getMidpoint().x - 300 + bfCamOffset.x;
				case 'mall':
					camFollow.y = boyfriend.getMidpoint().y - 200 + bfCamOffset.y;
				case 'school' | 'schoolEvil':
					camFollow.x = boyfriend.getMidpoint().x - 200 + bfCamOffset.x;
					camFollow.y = boyfriend.getMidpoint().y - 200 + bfCamOffset.y;
			}
	...
	private function keyShit():Void
	{
		...
		if ((holdArray.contains(true) || playerBotplay) && /*!boyfriend.stunned && */ generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && (holdArray[daNote.noteData] || playerBotplay))
					goodNoteHit(daNote);
			});
		}

		// PRESSES, check for note hits
		if (pressArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic && !playerBotplay)
		{
		...
		{
			if (pressArray[spr.ID] && !playerBotplay && spr.animation.curAnim.name != 'confirm')
				spr.animation.play('pressed');
			if (!holdArray[spr.ID] && !playerBotplay || playerBotplay && (spr.animation.curAnim.finished || playerTimerArray[spr.ID] <= 0) && !playerSusArray[spr.ID])
	...
	var playerSusArray:Array<Bool> = [false, false, false, false]; // for botplay
	var playerTimerArray:Array<Float> = [0, 0, 0, 0]; // Making the bot's arrow unglow quicker.
		...
		if (!note.wasGoodHit)
		{
			...
			playerStrums.forEach(function(spr:FlxSprite)
			{
				if (Math.abs(note.noteData) == spr.ID)
				{
					...
					if (playerBotplay)
					{
						playerTimerArray[note.noteData] = note.timePressing;
						if (note.noteStatus > 1)
							playerSusArray[note.noteData] = true;
						else
							playerSusArray[note.noteData] = false;
					}
				}
			});
	...
	function lightningStrikeShit():Void
	{
		...
		FlxTween.num(1, 0, 1, null, function (f:Float) { imageDuoShady.blendPercent = f; }); // I want cool effect now!!!!!!!
	}
